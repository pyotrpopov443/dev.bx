...
function formatMovieItem(int $pos, array $movie) : string
{
	$number = $pos + 1;
	$movieString = formatMovie($movie);
	return "$number. $movieString";
}
...
function formatMovie(array $movie) : string
{
	...
	$rating = $movie["rating"];
	return "$title ($year), $age+. Rating - $rating";
}

function printLine(string $messageToPrint)
{
	echo $messageToPrint . "\n";
}
